...
async function loadComments(){
    const commentsList = document.getElementByld("commentsList");
    ...
    querySnapshot.forEach(doc => {
        ...
        listItem.innerHTML= `
            <strong> ${data.nombre || 'Anónimo'}</strong>: ${data.comentario}
            <button onclick= "editComment('${doc.id}','${data.comentario}')">Editar</button>
        `;
...
async function editComment(commentId,oldComment){
    const newComment=promp("Edita tu comentario:",oldComment);
